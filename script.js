const sheetURL= 'https://docs.google.com/spreadsheets/d/e/2PACX-1vSRuic27PzRsa0c6bHFlsys1mzcdStUHaLrCQMll3G1Mfy-ufwfGsng1KbK7rtXXrrafNvYEhjAZWkg/pub?gid=0&single=true&output=csv'
...
async function carregarDados() {
  try {
    ...
    dados.forEach((linha) => {
      ...
      const nomeSalgado = linha[1];

      // Cria botão para WhatsApp
      const botao = document.createElement('a');
      botao.href = `https://wa.me/5599999999999?text=Olá! Gostaria de pedir um(a): ${encodeURIComponent(nomeSalgado)}`;
      botao.target = '_blank';
      botao.className = 'mt-4 inline-block bg-green-500 text-white font-semibold py-2 px-4 rounded hover:bg-green-600';
      botao.textContent = '📲 Comprar salgado';
